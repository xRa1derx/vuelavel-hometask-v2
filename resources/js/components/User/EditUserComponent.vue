            ...
            <form enctype="multipart/form-data">
              <div v-if="!cropper" class="row mb-3">
                <div class="info-wrapper d-flex">
                  <img
                    ref="imageSrc"
                    class="card-img-top rounded-circle p-3 w-50 h-100"
                    :src="`/uploads/avatars/${
                      currentUserAvatar || 'no-user-image.png'
                    }`"
                    alt="Card image cap"
                  />
                  <div
              ...
              >
                <template v-slot:default>Change avatar</template>
              </crop-avatar-component>
              <button
  ...
  methods: {
    ...
    onClassChange(classAttrValue) {
      ...
      if (!classList.includes("show")) {
        ...
        if (this.$refs.imageSrc) {
          this.$refs.imageSrc.src = `/uploads/avatars/${
    ...
    async updateUser() {
      ...
      if (this.avatar === null) {
        ...
        // if changing Data WITHOUT avatar
        await fetch(this.$refs.imageSrc.src)
          .then((res) => res.blob())
          .then((blob) => {
            data.append("avatar", blob);
          });
      } else {
        // if creating NEW avatar
        data.append("avatar", this.avatar);
      }
      axios.post(`/api/admin/users/${this.currentUserId}`, data).then(() => {
        {
    ...
    clearAvatar() {
      ...
      this.isChanged = true;
      this.clearAvatarBtn = false;
      this.$refs.imageSrc.src = "/uploads/avatars/no-user-image.png";
    },
  },
  watch: {
    currentUserName(val) {
